...
# Load data
# --------------------------------------------------------------------------
path <- "tidytuesday/data/2021/2021-03-18"
...
df_pic1 <- gms %>%
	...
	mutate(gamename = factor(gamename,
		levels = df_pic1$gamename[order(df_pic1$peak_sum, decreasing = TRUE)]))
...
ggplot(df_pic1, aes(x = gamename, y = peak_sum, fill = gamename)) +
	...
	theme(
		...
		axis.text.y = element_text(margin = margin(t = 0, r = 30, b = 0, l = 15)),
...
ggsave(file.path("viz", paste0("viz_", Sys.Date(), ".png")), width = ,
	height = )